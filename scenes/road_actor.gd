...
var current_rotation := 0.0 # For smooth rotation
...
func _physics_process(delta: float) -> void:
	...
	var orientation: Vector3 = agent.test_move_along_lane(move_dist + 0.001)
	...
	if next_pos != orientation:
		...
		var dir = (orientation - next_pos).normalized()
		
		# Calculate the target basis (rotation) towards the direction
		var target_basis = Basis.looking_at(dir, Vector3.UP)
		
		# Smooth interpolation for rotation using slerp
		# Delta is already normalized in time, so we use it for smooth interpolation
		var rotation_speed = 6.0  # You can adjust this to change rotation speed
		var smooth_rotation = global_transform.basis.slerp(target_basis, rotation_speed * delta)

		# Apply the smoothly interpolated rotation to the global transform
		global_transform.basis = smooth_rotation
